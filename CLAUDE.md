...
- `games.ts` — public routes: fuzzy `search` (strips stopwords/punctuation, ANDs remaining keywords across title/publisher with `LIKE`), `manual-entry` (find-or-create shelf by name, `COLLATE NOCASE`), `shelves` listing, and `init` (PIN-gated schema bootstrap for local dev).
...
- `UserSearch.tsx` (public) takes no `pin` prop and calls `/api/games/*` unauthenticated.
